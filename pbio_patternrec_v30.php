	...
	*/
	...
	public function getLocationDetail($ui_FirstBound,$ui_SecondBound,$ui_LocationSpecifier)
	{
		...
		foreach ($a as &$value)
		{
			...
			echo "getLocationDetail arrayElement: " .$value."<br>";
		...
		{
			//$this->arraySetLocationCriteria[] = $this->arrayWriteToExcel ;
	...
	public function setLocationCriteria_old($ui_LocationSpecifier)
	{
			//Set City, County, State Condition
			$locationSpecifier = $ui_LocationSpecifier;			
			
			echo "FUNCTION: setLocationCriteria. Printing array: ";
			$a = $this->arraySetLocationCriteria[0];
			print_r($this->arraySetLocationCriteria);
			
			foreach ($a as $value)
			{			
				$input = $value;
				echo "Location Specifier: " .$locationSpecifier."<br>";
				print_r($input);
				
				if (substr_count($input,',') == 4)
				{
					if($locationSpecifier == "state")
					{
						echo "Printing state: ";
						$output = $this->subtok($input,',',5,1);
					}
					elseif($locationSpecifier == "county")
					{
						echo "Printing state: ";
						$output = $this->subtok($input,',',4,1);
					}
					elseif($locationSpecifier == "city")
					{
						echo "Printing state: ";
						$output = $this->subtok($input,',',3,1);
					}
	...
	public function writeExcelSource($ui_ColInit,$ui_RowInit,$ui_SheetName,$ui_FileName,$ui_FileExtension)
	{
		...
		$columnArray = array_chunk(array_column($rowArray,'1'),1); //modify array_column column_key to output specified column for 2d array
		...
		//$objPHPExcel->getActiveSheet()->fromArray($rowArray, null, 'A2');	//2D array		
...
$records->setSearchCriteria("Ment");
