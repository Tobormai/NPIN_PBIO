...
class ExcelRecord
{
	...
	*/
	...
	{
		$this->regexHtmlTag = $newTag;
		echo "Printing arrayWriteToExcel column 0";
		$this->arrayUrlContent = array_column($this->arrayWriteToExcel,'0');
		print_r($this->arrayUrlContent);
		$a = $this->arrayUrlContent;
		...
		foreach ($a as $value)
		{
			...
			// escape special characters in the query
			//$regex = preg_quote($this->regexHtmlTag, '/');

			// finalise the regular expression, matching the whole line
			$regex = "~<blockquote\b[^>]*>(?:[^<]+|(?R)|<(?!/(?:blockquote|p)>))*</blockquote>~";

			// search, and store all matching occurences in $matches
			if(preg_match_all($regex, $urlCurrent, $matches))
			{
				echo "Found matches:\n";
				echo implode("\n", $matches[0]);  	
			...
			//$this->arrayWriteToExcel[] = $this->arrayUrlContent[0];	//array appended to end	
...
$ui_SecondBound = '.html';
print_r($records->getStringBetweenCriteria($ui_FirstBound,$ui_SecondBound));
$records->getUrlContent($newTag);
echo "Printing accession_date array urls";
print_r($records->matches);
